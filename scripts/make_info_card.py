...
ROWS = [
    ("host",),
    ("kv", "Now", "Co-Founder | CTO @ Rumy"),
    ("kv", "Prev", "Production Engineering Fellow @ Meta"),
    ("kv", "Also", "SWE Intern @ Athena AI (3M+ users)"),
    ("kv", "Edu", "B.Eng. Software Engineering, Concordia"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Langs", "JS, TS, Python, Java, C#, Kotlin, Dart"),
    ("kv", "Web", "React, Next.js, Tailwind, HTML/CSS"),
    ("kv", "Mobile", "React Native, Flutter, Expo, Unity"),
    ("kv", "Backend", "Node.js, Firebase, MySQL, WordPress"),
    ("kv", "Tools", "Vercel, Git, Linux, Oculus"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "2nd Place CareXR Hack - pediatric VR (solo)"),
    ("bul", "3rd Place BuildFast Hackathon (solo)"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">adam@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">adam</tspan><tspan fill="{MUTED}">@</tspan>'
